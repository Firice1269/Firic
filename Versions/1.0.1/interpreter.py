...
argv: list[str] = ["interpreter.py", "main.fi"]
    ...
    for line in lines:
        ...
        for character in characters:
            ...
            if character == deliminator and string == False:
                ...
                temporary_token = ""
            else: temporary_token += character

        # Append Tokens
        if len(temporary_token) != 0: tokens.append(temporary_token)

        # Check Types
        for token in tokens:
            # String
            if (value_check(token, 0) == '"' and value_check(token, -1) == '"') or (value_check(token, 0) == "'" and value_check(token, -1) == "'"): items.append(("string", token))

            # Keyword
            if re.match(r"[.a-z]+", token): items.append(("keyword", token))

            # Operation
            if token in operations: items.append(("operation", token))

            # Number
            if re.match(r"[.0-9]+", token): items.append(("number", token))
...
def parse(file: str):
    ...
    for line_index, line in enumerate(lines):
        ...
        for token_index, token in enumerate(line):
            ...
            if token[0] == "keyword":
                ...
                if token[1] not in keywords:
                    # Calling Functions
                    if token[1] in functions: python_code += functions[token[1]]
                    ...
                    # Declaring Variables
                        ...
                        else: python_code += f'variables["{token[1]}"] = {line[token_index + 1][1]}'
                    ...
                    # Assigning Variables
                        ...
                        if token[1] in variables:
                            ...
                            for i in line[token_index + 1:]: python_code += f" {i[1]} "
                        else:
                            error = True
                    ...
                    # Referencing Variables
                        ...
                        if function: python_code = python_code.replace(token[1], f'variables["{token[1]}"]')
                        else:
                            if variables[token[1]] == str(variables[token[1]]): python_code = python_code.replace(token[1], f'"{variables[token[1]]}"')
                            else: python_code = python_code.replace(token[1], str(variables[token[1]]))
        ...
        if function and value_check(line, 0)[1] != "func" and python_code != "": function_code += f"{python_code.replace('"', '\\"')}; "
        elif error: break
        else: exec(python_code)


if argv[-1].split(".")[1] == file_extension: parse(argv[-1])
